<template>
  <div class="browseResult">
    <div class="browseResult-inner">
      <div class="blast">
        <!-- <el-header>Analysis from cellranger</el-header> -->
        <div style="margin-bottom:20px;width:100%;height:auto;background: rgba(255, 85, 85, 0.06);color:#FF5555;font-size: 14px;line-height: 24px;padding:13px;box-sizing: border-box;">
          <span class="speaker"></span>
          Please enter the email address to view the detaied result of your jobs.
        </div>
...
export default {
  ...
  methods: {
    ...
    viewHandle(params) {
      let stepList = []
      if (params.row.source == 'cell_ranger') {
      ...
      stepList.forEach((item, index) => {
        if (params.row.status == 'Failure') {
          if (params.row.current_step == item.step) {
            this.$confirm('The job failed to run successfully. Would you like to resubmit the task?', 'Tips', {
              confirmButtonText: 'Confirm',
              cancelButtonText: 'Cancel',
              type: 'warning'
            }).then(() => {
              this.$router.push({
                path: '/' + stepList[0].link,
                query: {
                  uuid: params.row.uuid,
                  id: params.row.id,
                  species_name: params.row.species_name,
                  from: params.row.source,
                  enterResults: item.enterResults,
                  transcriptome: params.row.transcriptome,
          ...
          if (params.row.current_step == item.step) {
            this.$router.push({
              ...
              query: {
                ...
                from: params.row.source,
                enterResults: item.enterResults,
                is_run_qc: params.row.is_run_qc
              }
            })
          }
        }
