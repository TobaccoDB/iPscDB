<template>
  <div class="analysisFromSampleQC">
    <div class="analysisFromSampleQC-inner">
      <div class="blast">
        <el-header>Analysis from Sample QC</el-header>
          ...
          <el-table :cell-style="cellstyle" :data="tableData" :loading="loading" border style="width: 100%">
            ...
            <el-table-column fixed="right" label="Operation" width="120" align="center">
              <!-- <template slot-scope="scope">
                <i v-if="scope.row.status == 'SUCCESS'" class="el-icon-view pointer" style="padding: 0 5px; color: #24a461" @click="viewHandle(scope)"></i>
                <i class="el-icon-delete pointer" style="padding: 0 5px; color: #24a461" @click="deleteHandle(scope)"></i>
              </template> -->
              <template slot-scope="scope">
                <!-- <i v-if="scope.row.status == 'SUCCESS'&&scope.row.email != 'XXX'" class="el-icon-view pointer" style="padding: 0 5px; color: #24a461"
                  @click="viewHandle(scope)"></i>
                <i v-if="scope.row.email != 'XXX'" class="el-icon-delete pointer" style="padding: 0 5px; color: #24a461" @click="deleteHandle(scope)"></i>
                <span v-if="scope.row.email == 'XXX'">XXX</span> -->
  ...
  methods: {
    ...
    viewHandle(params) {
      ...
      if (params.row.status == 'Failure') {
        stepList = [
          { name: 'Sample QC', link: 'SampleSampleQC', step: 'sample_qc', enterResults: false },
        ...
        stepList = [
          { name: 'Sample QC', link: 'SampleSampleQC', step: 'sample_qc', enterResults: true },
          { name: 'Data Process', link: 'SampleSampleQC', step: 'data_process', enterResults: true },
