<template>
  <div class="SampleQC">
    <div class="SampleQC-inner">
      ...
      <el-dialog :visible.sync="progressDialogVisible" :close-on-click-modal="false" custom-class="no-header-dialog" width="500px"
        center>
        <span style="color: rgb(51, 51, 51);font-family: 思源黑体;font-size: 14px;line-height: 20px;">The task has been submitted to the background for execution. You can wait for the completion of the task here to
          view the result, or click close go to the home page to wait for the result.</span>
        <el-progress style="margin:29px 0 15px 0;font-size: 14px;" define-back-color="#E7EFEA" text-color="#fff" :text-inside="true"
          color="#24A461" :stroke-width="20" :percentage="30" status="success"></el-progress>
        ...
        <span slot="footer" class="dialog-footer">
          <el-button style="color: rgb(36, 164, 97);font-family: 思源黑体;font-size: 14px;line-height: 20px;" type="text" @click="progressDialogVisible = false">close</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
...
<script>
import { sample_qc_pt_mt, sqmple_qc_data_show, qc_filter_data_show, sample_qc, qc_filter_data_result } from '@/api/analysis'
...
export default {
  ...
  mounted() {
    ...
    this.init()
    if (this.$route.query.step == 'data_process') {
