<template>
  <div class="monocleDifferential">
    <div class="crumbs">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to="{ path: '/monocle' }">Developmental trajectory</el-breadcrumb-item>
        <el-breadcrumb-item>Differential genes</el-breadcrumb-item>
      </el-breadcrumb>
    </div>
    <div class="monocleDifferential-inner" :style="{minHeight: contentHeight}" element-loading-text="loading" element-loading-background="#fff">
      <div class="searchJump">
        ...
        <el-select v-model="searchJump" size="large" style="width:300px;" multiple filterable remote reserve-keyword placeholder="Please enter keywords"
...
<script>
import { monocle_heatmap_down, monocle_heatmap_png } from "@/api/api";
import jgTable from '@/components/jgTable/index'

export default {
  name: "monocleDifferential",
  components: {
    jgTable
  },
  data() {
  ...
  methods: {
    ...
    search() {
      ...
      monocle_heatmap_png({
        ...
        if (res.code == 200) {
          this.staticUrl = res.data.heatmap_png
          this.loading = false
        }
      });
    }
