<template>
  <div class="sampleQC" :style="{ minHeight: '390px'}">
    <div class="sampleQC-inner">
      <div class="scsa" v-loading="loading">
        ...
        <el-row :gutter="20">
          <el-col :span="12">
            <div>
              <el-form ref="form1" :model="sampleQC_form" :inline="false" :rules="rules">
...
export default {
  ...
  methods: {
    init() {
      ...
      let params = {
        lit_id: this.$route.query.lit_id || 'LT24'
      }
      cell_identification_reference_detail(params).then(res => {
        if (res.code == 200) {
          this.detailData = res.data.results[0]
          this.tissue = this.detailData.title.split('-')[1]
          this.sampleQC_form.species = res.data.results[0].species_name
        }
      });
    },
    fileTypeChange(val) {
