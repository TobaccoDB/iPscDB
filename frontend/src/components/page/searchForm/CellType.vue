        ...
        <div class="marker-content2">
            ...
            <el-table :data="tableData2" style="width: 100%" v-loading="count2Loading">
                ...
                <el-table-column prop="tissue" label="Tissue" align="center"
                    ><template slot-scope="scope">
                        <span>{{ $route.query.tissue_name }}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="species" label="Cell" align="center">
                    <template slot-scope="scope">
                        <span>{{ $route.query.cell_name }}</span>
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="classic_marker" label="Classic Marker" align="center">
                    <template slot-scope="scope">
                        <span class="tableItemTableIcon" style="font-size: 26px" :class="scope.row.classic_marker == '1' ? 'el-icon-star-on tableItemTableIconActive' : 'el-icon-star-off '"></span>
                    </template>
                </el-table-column>
            </el-table>
            <div class="TableItemBody">
...
import { marker_word_data, search_word_cloud_list, search_cell_type_table } from '@/api/api'
export default {
    data() {
        return {
            tableData1: [],
    ...
    mounted() {
        ...
        let tissue_name = this.$route.query.tissue_name
        let cell_name = this.$route.query.cell_name

        console.log(species_name, tissue_name, cell_name)
        // if (!species_name || !tissue_name || !cell_name) {
        //     this.$router.back()
        // }
        this.getWord()
        this.getTableTop()
    ...
    methods: {
        ...
        getWord() {
            ...
            marker_word_data({
                ...
                tissue_name: this.$route.query.tissue_name,
                cell_name: this.$route.query.cell_name
            })
                .then((res) => {
                    if (res.code == 200) {
        ...
        downLoad() {
            let url = `${this.baseUrl}/api/v1/search_cell_type_table/search_clee_type_down?species_name=${this.$route.query.species_name}&tissue_name=${this.$route.query.tissue_name}&cell_name=${this.$route.query.cell_name}`
            window.open(url)
