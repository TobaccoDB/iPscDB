  ...
  <div class="search-list">
    <div class="crumbs">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        ...
        <el-breadcrumb-item v-else :to="{ path: '/blast' }">Blast</el-breadcrumb-item>
    ...
    <div class="search-list-inner">
      ...
      <div class="search-list-table">
        ...
        <el-table :data="tableData" class="table" ref="multipleTable" stripe header-cell-class-name="table-header" :header-cell-style="headerStyle"
          :row-style="rowStyle" :row-class-name="tableRowClassName" @cell-click="handlecell" :cell-style="cellstyle">
          >
          <el-table-column v-for="(item, index) in tableDataItem" :width="item.width" :key="index" :prop="item.prop" align="center"
      ...
      <div style="margin-top:20px;">
        ...
        <div v-for="(item, index) in itemsBottom" :key="item.hits + index" style="color:#666;font-size:14px;">
          ...
          <p class="bottom_p">
            Score = {{item.score1}} bits ({{item.bits1}})， Expect = {{item.Expect1}},Method: Compositional matrix adjus
          </p>
...
export default {
  ...
  methods: {
    ...
    getData() {
      this.tableData = JSON.parse(sessionStorage.getItem("tableParams3"))
      ...
      this.itemsBottom.forEach(item => {
        ...
        for (let i = 0; i < Math.ceil(item.query.length / 60); i++) {
          this.querySbjct.push({
            query: `<span style="display:inline-block;width:90px;">Query:${Number(1) + i * 60}</span>` + `<span style="display:inline-block;text-align:justify;width:auto;">${item.query.substr(i * 60, 60)}</span>` + `<span style="display:inline-block;padding-left:10px;">${i < Math.ceil(item.query.length / 60) - 1 ? (i + 1) * 60 : item.query.length}</span><br />`,
            // match: `${item.match} <br />`,
            sbjct: `<span style="display:inline-block;width:90px;">Sbjct:${Number(1) + i * 60}</span>` + `<span style="display:inline-block;text-align:justify;width:auto;">${item.sbjct.substr(i * 60, 60)}</span>` + `<span style="display:inline-block;padding-left:10px;">${i < Math.ceil(item.query.length / 60) - 1 ? (i + 1) * 60 : item.query.length}</span><br />`,
          })
        }
      })
    },
    ...
    cellstyle({ row, column, rowIndex, columnIndex }) {
      if (columnIndex === 1) {
        return { color: "rgb(34, 173, 99)", cursor: "pointer" };
        // return { color: '#3366CC', cursor: 'pointer', textDecoration: 'underline' }
      }
